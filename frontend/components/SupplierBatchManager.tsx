...
type Batch = {
  ...
  brin_request_payload: Record<string, unknown> | null;
  brin_response_payload: Record<string, unknown> | null;
...
export default function SupplierBatchManager() {
  ...
  const fetchBatches = async () => {
    try {
      ...
      console.error(err);
      setError(toMessage(err) || 'Failed to load batches');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setError(toMessage(err) || 'Failed to create batch');
    } finally {
      setCreating(false);
    }
  };
  ...
  const submitToBrin = async (id: number) => {
    try {
      ...
      console.error(err);
      setError(toMessage(err) || 'Failed to submit QC');
    }
  };
  ...
  const processBrin = async (id: number) => {
    try {
      ...
      console.error(err);
      setError(toMessage(err) || 'Failed to process BRIN verification');
    }
  };
